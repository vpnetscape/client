...
func dnsWatch() {
	...
	for {
		...
		vpn, _ := utils.GetScutilKey("/Network/vpnetscape/DNS")
		...
		if strings.Contains(vpn, "No such key") {
			...
			err = utils.CopyScutilKey(
				fmt.Sprintf("/Network/vpnetscape/Connection/%s", connIds[0]),
				"/Network/vpnetscape/DNS",
			)
			if err != nil {
				logrus.WithFields(logrus.Fields{
					"error": err,
		...
		if !reflect.DeepEqual(vpnDomains, globalDomains) ||
			...
			if reset {
				...
				if err != nil {
					...
					err = utils.CopyScutilDns("/Network/vpnetscape/DNS")
					if err != nil {
						logrus.WithFields(logrus.Fields{
							"error": err,
