...
func RestoreScutilDns() (err error) {
	...
	serviceKey := fmt.Sprintf("/Network/vpnetscape/Restore/%s", serviceId)
...
func BackupScutilDns() (err error) {
	...
	if strings.Contains(data, "No such key") ||
		strings.Contains(data, "vpnetscape : true") {
		return
	}

	err = CopyScutilKey(serviceKey,
		fmt.Sprintf("/Network/vpnetscape/Restore/%s", serviceId))
	if err != nil {
		return
	}
...
func GetScutilConnIds() (ids []string, err error) {
	...
	for _, line := range strings.Split(string(output), "\n") {
		if !strings.Contains(line, "State:/Network/vpnetscape/Connection/") {
			continue
		}

		spl := strings.Split(line, "State:/Network/vpnetscape/Connection/")
		if len(spl) == 2 {
...
func ClearScutilKeys() (err error) {
	...
	for _, line := range strings.Split(string(output), "\n") {
		if !strings.Contains(line, "State:/Network/vpnetscape") {
			continue
		}

		if strings.Contains(line, "State:/Network/vpnetscape/Restore") {
			continue
		}
...
func GetAuthPath() (pth string) {
	if runtime.GOOS == "windows" {
		pth = filepath.Join("C:\\", "ProgramData", "vpnetscape")
...
func GetLogPath() (pth string) {
	if runtime.GOOS == "windows" {
		...
		pth = filepath.Join(pth, "vpnetscape.log")
	} else {
		pth = filepath.Join(string(filepath.Separator),
			"var", "log", "vpnetscape.log")
	}
...
func GetTempDir() (pth string, err error) {
	if runtime.GOOS == "windows" {
		...
		pth = filepath.Join(string(filepath.Separator), "tmp", "vpnetscape")
